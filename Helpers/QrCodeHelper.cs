...
public class QrCodeHelper
{
    ...
    public static byte[] GenerateQrCodeWithLogoPngBytes(string text, string logoPath)
    {
        ...
        if (File.Exists(logoPath))
        {
            ...
            // Draw the logo on the QR Code with its original colors
            qrImage.Mutate(ctx =>
            {
                var options = new DrawingOptions
                {
                    // Set alpha value to 1 to avoid transparency blending
                    GraphicsOptions = new GraphicsOptions { Antialias = true }
                };
                ctx.DrawImage(logoImage, new SixLabors.ImageSharp.Point(xPos, yPos), 1f); // Correctly use `Point` and alpha channel
            });
        }
