...
namespace QuilvianSystemBackendDev.Areas.HRD.MasterData.Controllers
{
    ...
    [ApiController]
    public class MasterSoalController : ControllerBase
    {
        private readonly ApplicationDbContext _context;
        public MasterSoalController(ApplicationDbContext context) => _context = context;
        ...
        public async Task<IActionResult> GetList(int page = 1, int perPage = 10)
        {
            ...
            var query =
                from m in _context.Set<MasterSoal>()
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<MasterSoal>> GetById(Guid id)
        {
            var data = await _context.Set<MasterSoal>().FindAsync(id);
            if (data == null) return NotFound();
            return data;
        }

        [HttpPost]
        public async Task<ActionResult<MasterSoal>> Create([FromBody] MasterSoalVM vm)
        {
            var entity = new MasterSoal
            {
                SoalId = Guid.NewGuid(),
                Soal = vm.Soal,
                KategoriTest = vm.KategoriTest,
                Keterangan = vm.Keterangan,
                CreateDateTime = DateTimeOffset.UtcNow
            };
            _context.Add(entity);
            await _context.SaveChangesAsync();
            return CreatedAtAction(nameof(GetById), new { id = entity.SoalId }, entity);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] MasterSoalVM vm)
        {
            var entity = await _context.Set<MasterSoal>().FindAsync(id);
            if (entity == null) return NotFound();

            entity.Soal = vm.Soal;
            entity.KategoriTest = vm.KategoriTest;
