...
namespace QuilvianSystemBackendDev.Areas.ManajemenKesehatan.MasterData.Controllers
{
    ...
    public class KunjunganOnlineController : Controller
    {
        private readonly ApplicationDbContext _applicationDbContext;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly SignInManager<ApplicationUser> _signInManager;

        private readonly ILogger<KunjunganOnlineController> _logger;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public KunjunganOnlineController
        (
            ApplicationDbContext context,
            UserManager<ApplicationUser> userManager,
            SignInManager<ApplicationUser> signInManager,

            ILogger<KunjunganOnlineController> logger,
        ...
        [HttpPost]
        public async Task<IActionResult> CreateKunjunganOnline([FromBody] KunjunganOnlineViewModel request)
        {
            if (request == null )
            {
            ...
            try
            {
                ...
                if (!isGuest)
                {
                    ...
                    if (tipeGuestId == Guid.Empty)
                    {
                        return BadRequest(new { message = "Tipe user 'Guest' tidak ditemukan di tabel MstTipeUsers." });
                    }

                    // Buat ApplicationUser (jika Anda pakai Identity)
                    var userLogin = new ApplicationUser
                    {
                        ...
                        PhoneNumber = "08122222254",
                        IsActive = true
                    };

                    // Tambahkan jika Anda menggunakan UserManager
                    // await _userManager.CreateAsync(userLogin, "DefaultPassword123!");

                    var userActiveId = Guid.NewGuid();

                    var user = new UserActive
                    {
                        UserActiveId = userActiveId,
                        UserActiveCode = "GUEST123",
                        FullName = "GUEST",
                        IdentityNumber = "GUEST1234567890",
                        PlaceOfBirth = "Online",
                        DateOfBirth = DateTime.SpecifyKind(DateTime.ParseExact("2000-10-10", "yyyy-MM-dd", null), DateTimeKind.Utc),
                        Gender = "Tidak Diketahui",
                        Address = "Online",
                        Handphone = "08122222254",
                        Email = "guest@guest.com",
                        IsActive = true,
                        CreateDateTime = DateTimeOffset.UtcNow,
                        CreateBy = userActiveId, // Atau system ID jika ada
                        TipeUserId = tipeGuestId
                    };
                ...
                // Ambil UserActiveId untuk user GUEST
                var guestId = _applicationDbContext.UserActives
                    .Where(u => u.FullName.ToLower() == "guest")
                    .Select(u => u.UserActiveId)
                    .FirstOrDefault();
                ...
                var isAlreadyRegistered = _applicationDbContext.Kunjungans.Any(k =>
                    ...
                    k.NoRekamMedis == request.NoRekamMedis &&
                ...
                // cari pasien berdasarkan NoRekamMedis
                var pasienRM = _applicationDbContext.PendaftaranPasienBarus
                    .FirstOrDefault(p => p.NoRekamMedis == request.NoRekamMedis 
                    || p.NoIdentitas == request.NIK);
                if (pasienRM == null)
                {
                    return BadRequest(new { message = "Pasien dengan No Rekam Medis atau NIK tersebut tidak ditemukan." });
                }

                var newKunjungan = new Kunjungan
                {
                    KunjunganID = Guid.NewGuid(),
                    PasienId = pasienRM.PendaftaranPasienBaruId,
