...
namespace QuilvianSystemBackendDev.Areas.ManajemenKesehatan.MasterData.Controllers
{
    ...
    public class JabatanController : Controller
    {
        ...
        [FromQuery, JsonConverter(typeof(StringEnumConverter))] PeriodeFilter? periode = null)
        {
            ...
            var query = _applicationDbContext.Jabatans.Where(a => a.IsDelete == false).AsQueryable();

            // 🔍 Filter berdasarkan search
            if (!string.IsNullOrWhiteSpace(search))
            {
                query = query.Where(u => u.KodeJabatan.Contains(search) && u.IsDelete == false ||
                                         u.NamaJabatan.Contains(search) && u.IsDelete == false);
            }
