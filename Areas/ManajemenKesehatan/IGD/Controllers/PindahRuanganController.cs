...
namespace QuilvianSystemBackendDev.Areas.ManajemenKesehatan.IGD.Controllers
{
    ...
    public class PindahRuanganController : Controller
    {
        ...
        public async Task<IActionResult> GetAll(int page = 1, int perPage = 10)
        {
            ...
            // Query data
            var query = (from a in _applicationDbContext.PindahRuangans
        ...
        {
            var listdata = _applicationDbContext.PindahRuangans.Find(id);
            if (listdata == null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PindahRuanganViewModel vm)
        {
            ...
            try
            {
                ...
                // **Buat Data Baru**
                var data = new PindahRuangan
                {
                    PindahRuanganId = Guid.NewGuid(),
                    UnitId = vm.UnitId,
                    KamarId = vm.KamarId,
                    TglAwal = vm.TglAwal,
                    TglAkhir = vm.TglAkhir,
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] PindahRuanganViewModel vm)
        {
            if (vm == null || !ModelState.IsValid)
            {
                return BadRequest(new { message = "Data tidak valid." });
            ...
            try
            {
                ...
                // **Cek apakah data PindahRuangan ada**
                var data = await _applicationDbContext.PindahRuangans
                    .FirstOrDefaultAsync(x => x.PindahRuanganId == id && (x.IsDelete == false || x.IsDelete == null));
                ...
                // **Update Data**
                data.UnitId = vm.UnitId ?? data.UnitId;
                data.KamarId = vm.KamarId ?? data.KamarId;
                data.TglAwal = vm.TglAwal ?? data.TglAwal;
                data.TglAkhir = vm.TglAkhir ?? data.TglAkhir;
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                ...
                // **Cari Data**
                var data = await _applicationDbContext.PindahRuangans.FindAsync(id);
                if (data == null)
                {
                    return NotFound(new { message = "Data tidak ditemukan." });
                }
