...
namespace QuilvianSystemBackendDev.Areas.ManajemenKesehatan.Pendaftaran.Controllers
{
    ...
    public class PendaftaranPasienMCUController : Controller
    {
        private readonly ApplicationDbContext _context;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly SignInManager<ApplicationUser> _signInManager;

        private readonly ILogger<PendaftaranPasienMCUController> _logger;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public PendaftaranPasienMCUController
            (ApplicationDbContext context, 
            UserManager<ApplicationUser> userManager, 
            SignInManager<ApplicationUser> signInManager, 
            ILogger<PendaftaranPasienMCUController> logger, 
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllPdfPasienMCU(int page = 1, int perPage = 10)
        {
            // validasi pagging
            if (page < 1) page = 1;
            ...
            // Query data
            var query = from a in _context.PendaftaranPasienMCUs
                        ...
                        select new
                        {
                            ...
                            PendaftaranPasienMCUId = a.PendaftaranPasienMCUId,
                            KodePdfPasienMCU = a.KodePdfPasienMCU,
                            PasienId = a.PasienId,
        ...
        {
            var record = await _context.PendaftaranPasienMCUs.FindAsync(id);
            if (record == null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PendaftaranPasienMCUViewModel vm)
        {
            ...
            try
            {
                ...
                var lastCode = _context.PendaftaranPasienMCUs
                    .Where(d => d.CreateDateTime.Date == dateNow.Date)
                    .OrderByDescending(k => k.KodePdfPasienMCU)
                    .FirstOrDefault();

                string kode;
                if (lastCode == null)
                {
                    kode = $"MCU{setDateNow}0001";

                }
                else
                {
                    var lastCodeTrim = lastCode.KodePdfPasienMCU.Substring(3, 6);
                    ...
                    {
                        kode = $"MCU{setDateNow}" + (Convert.ToInt32(lastCode.KodePdfPasienMCU.Substring(9)) + 1).ToString("D4");
                    }
                }

                // Cek Duplikasi
                var isDuplicate = _context.PendaftaranPasienMCUs
                    .Any(c => c.KodePdfPasienMCU == kode);

                if (ModelState.IsValid)
                {
                    var data = new PendaftaranPasienMCU
                    {
                        PendaftaranPasienMCUId = Guid.NewGuid(),
                        KodePdfPasienMCU = kode,
                    ...
                    _context.PendaftaranPasienMCUs.Add(data);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] PendaftaranPasienMCUViewModel vm)
        {
            if (vm == null || !ModelState.IsValid)
            {
                return BadRequest(new { message = "Data tidak valid." });
            ...
            try
            {
                ...
                // cari data
                var data = _context.PendaftaranPasienMCUs.Find(id);
                if (data == null)
                {
                ...
                _context.PendaftaranPasienMCUs.Update(data);
                _context.SaveChanges();
        ...
        [HttpGet("paged")]
        public IActionResult PagedPendaftaranPasienMCU(
        int page = 1,
        int perPage = 10,
        string? search = null,
