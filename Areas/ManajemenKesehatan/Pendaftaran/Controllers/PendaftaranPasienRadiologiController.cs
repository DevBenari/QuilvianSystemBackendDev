...
namespace QuilvianSystemBackendDev.Areas.ManajemenKesehatan.Pendaftaran.Controllers
{
    ...
    public class PendaftaranPasienRadiologiController : Controller
    {
        private readonly ApplicationDbContext _context;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly SignInManager<ApplicationUser> _signInManager;

        private readonly ILogger<PendaftaranPasienRadiologiController> _logger;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public PendaftaranPasienRadiologiController
            (ApplicationDbContext context, 
            UserManager<ApplicationUser> userManager, 
            SignInManager<ApplicationUser> signInManager, 
            ILogger<PendaftaranPasienRadiologiController> logger, 
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllPdfPasienRadiologi(int page = 1, int perPage = 10)
        {
            // validasi pagging
            if (page < 1) page = 1;
            ...
            // Query data
            var query = from a in _context.PendaftaranPasienRadiologis
                        ...
                        select new
                        {
                            ...
                            PendaftaranPasienRadiologiId = a.PendaftaranPasienRadiologiId,
                            KodePdfPasienRadiologi = a.KodePdfPasienRadiologi,
                            PasienId = a.PasienId,
        ...
        {
            var record = await _context.PendaftaranPasienRadiologis.FindAsync(id);
            if (record == null)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PendaftaranPasienRadiologiViewModel vm)
        {
            ...
            try
            {
                ...
                var lastCode = _context.PendaftaranPasienRadiologis
                    .Where(d => d.CreateDateTime.Date == dateNow.UtcDateTime.Date)
                    .OrderByDescending(k => k.KodePdfPasienRadiologi)
                    .FirstOrDefault();

                string kode;
                if (lastCode == null)
                {
                    kode = $"RAD{setDateNow}0001";

                }
                else
                {
                    var lastCodeTrim = lastCode.KodePdfPasienRadiologi.Substring(3, 6);
                    ...
                    {
                        kode = $"RAD{setDateNow}" + (Convert.ToInt32(lastCode.KodePdfPasienRadiologi.Substring(9)) + 1).ToString("D4");
                    }
                }

                // Cek Duplikasi
                var isDuplicate = _context.PendaftaranPasienRadiologis
                    .Any(c => c.KodePdfPasienRadiologi == kode);

                if (ModelState.IsValid)
                {
                    var data = new PendaftaranPasienRadiologi
                    {
                        PendaftaranPasienRadiologiId = Guid.NewGuid(),
                        KodePdfPasienRadiologi = kode,
                    ...
                    _context.PendaftaranPasienRadiologis.Add(data);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] PendaftaranPasienRadiologiViewModel vm)
        {
            if (vm == null || !ModelState.IsValid)
            {
                return BadRequest(new { message = "Data tidak valid." });
            ...
            try
            {
                ...
                // cari data
                var data = _context.PendaftaranPasienRadiologis.Find(id);
                if (data == null)
                {
                ...
                _context.PendaftaranPasienRadiologis.Update(data);
                _context.SaveChanges();
        ...
        [HttpGet("paged")]
        public IActionResult PagedPdfPasienRadiologi(
        int page = 1,
        int perPage = 10,
        string? search = null,
